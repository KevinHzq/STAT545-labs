...
```
## Error in levels(x): object 'h_gap' not found
```
...
```
## Error in h_gap %>% droplevels(): could not find function "%>%"
```
...
```
## Error in ggplot(h_gap, aes(x = year, y = lifeExp, color = fct_reorder2(country, : could not find function "ggplot"
```
