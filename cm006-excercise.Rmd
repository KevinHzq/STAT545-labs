...
```{r}
select(gapminder, year, lifeExp, country, continent)
```
...
```{r}
gapminder %>%
  mutate(gdpBill = round(gdpPercap*pop/1000000000))
```
...
```{r}
gapminder %>%
  mutate(lifeExp = if_else(country == "Canada" & year == 1957, 70, lifeExp))
```
