...
gapminder %>% 
  ...
  geom_line(alpha = 0.2) +
  scale_color_discrete("", labels = c("Others", "Rwanda")) # change legend
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  facet_wrap(~ continent, scales = "free_y") + #split up plot into panels of groups
  theme_classic()
```
...
gapminder %>% 
  ...
  facet_wrap(~ continent, nrow = 1) + #all in one row
  geom_point(alpha = 0.5, shape = 21) + #can use fill with shape 21
  scale_x_log10(labels = scales::comma_format()) +
  scale_size_area() #size by different area instead of diameter
```
